...
(function () {
  ...
  function apply(func, thisArg, args) {
    // 根据参数长度调用 func 的 call 方法，在少于三个参数的情况下 call 的性能高于 apply
    switch (args.length) {
      case 0:
  ...
  function arrayAggregator(array, setter, iteratee, accumulator) {
    ...
    /**
     * while 循环的性能高于 for 循环
     * 性能优势 - while 循环在大多数 JavaScript 引擎中执行更快
     * 内存效率 - 减少变量创建和内存分配
     * 代码一致性 - 整个库保持统一的循环模式
     * 历史原因 - 早期就确立了这种模式，并一直保持
     * 引擎优化 - JavaScript 引擎对 while 循环的优化更好
     */
    while (++index < length) {
  ...
  function arrayFilter(array, predicate) {
    ...
    while (++index < length) {
      ...
      if (predicate(value, index, array)) {
        // 进过测试，索引赋值（60ms）性能优于 push（70ms） -- 循环10000000 
  ...
  function arrayIncludesWith(array, value, comparator) {
    ...
    while (++index < length) {
      // 允许自定义  comparator 来进行 includes 的判断
  ...
  function arrayReduce(array, iteratee, accumulator, initAccum) {
    var index = -1,
      length = array == null ? 0 : array.length;
    // 感觉这个参数没啥太大的必要 initAccum（es 标准 api 已经去掉了）
    if (initAccum && length) {
